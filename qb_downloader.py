...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║          KCT QUESTION BANK — BULK DOWNLOADER & ORGANIZER v2.0              ║
║  High-speed async downloader with live stats, PDF parsing & auto-sorting   ║
╚══════════════════════════════════════════════════════════════════════════════╝

Downloads PDFs from: https://library.kct.ac.in/opac-tmpl/bootstrap/QB/QB{id}.pdf
Traverses IDs 13000–14000, extracts course codes starting with 'U',
...
# Department is always first 2 letters — no mapping needed, just slice.
...
def extract_course_code(pdf_bytes: bytes) -> str | None:
    ...
    Extremely fast — processes in-memory without disk I/O.
    """
    try:
...
def get_dept_from_code(course_code: str) -> tuple[str, str]:
    ...
    U18MEE0034 → regulation='18', dept='ME'  (NOT 'MEE')
    U18ECI645  → regulation='18', dept='EC'  (NOT 'ECI')
    """
...
def build_dashboard(stats: Stats, progress: Progress) -> Table:
    ...
    main_table = Table(
        box=box.DOUBLE_EDGE,
        title="[bold cyan]📚 KCT Question Bank Downloader[/bold cyan]",
    ...
    grid.add_column("📊 Metric", style="bold white", ratio=2)
    ...
    grid.add_row(
        "✅ Downloaded", f"[green]{stats.downloaded}[/green]",
        "❌ Not Found (404)", f"[yellow]{stats.not_found}[/yellow]",
    )
    grid.add_row(
        "📁 Organized (U-codes)", f"[cyan]{stats.organized}[/cyan]",
        "⏭️  Skipped (no code)", f"[dim]{stats.skipped_no_code}[/dim]",
    )
    grid.add_row(
        "🔄 Retried", f"[yellow]{stats.retried}[/yellow]",
        "💀 Failed", f"[red]{stats.failed}[/red]",
    )
    grid.add_row(
        "⚡ Speed", f"[magenta]{stats.speed_mbps:.2f} MB/s[/magenta]",
        "📄 Papers/sec", f"[magenta]{stats.papers_per_sec:.1f}[/magenta]",
    )
    grid.add_row(
        "💾 Downloaded", f"[blue]{stats.bytes_downloaded / (1024*1024):.1f} MB[/blue]",
        "⏱️  Elapsed", f"[white]{mins}m {secs}s[/white]",
    )
    grid.add_row(
        "📈 Progress", f"[bold yellow]{stats.progress_pct:.1f}%[/bold yellow]",
        "🔢 Total Range", f"[white]{stats.total}[/white]",
    )
    ...
    if stats.dept_counts:
        dept_table = Table(
            box=box.ROUNDED,
            title="[bold]🏛️  Department Breakdown[/bold]",
            expand=True,
            show_header=True,
        )
    ...
    if stats.reg_counts:
        ...
        main_table.add_row(Text.from_markup("📋 Regulations: " + " │ ".join(reg_parts)))

    # Recent errors
    if stats.errors:
        last_errors = stats.errors[-3:]
        err_text = " | ".join(f"[red]{e}[/red]" for e in last_errors)
        main_table.add_row(Text.from_markup(f"⚠️  Recent errors: {err_text}"))
...
async def run():
    ...
    console.print()
    pre_table = Table(
        title="[bold cyan]🚀 PRE-DOWNLOAD SUMMARY[/bold cyan]",
        box=box.DOUBLE_EDGE,
        show_header=False,
        expand=False,
        padding=(0, 2),
    ...
    pre_table.add_row("📌 URL Pattern", BASE_URL.format(qid="XXXXX"))
    pre_table.add_row("🔢 Range", f"QB{START_ID} → QB{END_ID}")
    pre_table.add_row("📄 Total PDFs to check", str(total_ids))
    pre_table.add_row("⚡ Concurrency", str(MAX_CONCURRENT))
    pre_table.add_row("🔄 Max retries/PDF", str(MAX_RETRIES))
    pre_table.add_row("📂 Output directory", str(OUTPUT_DIR))
    pre_table.add_row("🎯 Filter", "Course codes starting with 'U' only")
    pre_table.add_row("📁 Folder structure", "{Reg}Regulation/{Dept} Department/{Code}.pdf")
    ...
    post_table.add_row("📄 Total PDFs checked", str(stats.total))
    post_table.add_row("✅ Successfully downloaded", str(stats.downloaded))
    post_table.add_row("📁 Organized (U-code papers)", f"[bold cyan]{stats.organized}[/bold cyan]")
    post_table.add_row("❌ Not found (404)", str(stats.not_found))
    post_table.add_row("⏭️  Skipped (no course code)", str(stats.skipped_no_code))
    post_table.add_row("⏭️  Skipped (non-U code)", str(stats.skipped_non_u))
    post_table.add_row("💀 Failed", f"[red]{stats.failed}[/red]")
    post_table.add_row("🔄 Total retries", str(stats.retried))
    post_table.add_row("💾 Total data downloaded", f"{stats.bytes_downloaded / (1024*1024):.2f} MB")
    post_table.add_row("⏱️  Total time", f"{mins}m {secs}s")
    post_table.add_row("⚡ Avg speed", f"{stats.speed_mbps:.2f} MB/s")
    post_table.add_row("📄 Avg papers/sec", f"{stats.papers_per_sec:.1f}")
    ...
    if stats.dept_counts:
        console.print()
        dept_table = Table(
            title="[bold cyan]🏛️  PAPERS BY DEPARTMENT[/bold cyan]",
            box=box.ROUNDED,
            show_header=True,
        )
    ...
    if stats.reg_counts:
        console.print()
        reg_table = Table(
            title="[bold cyan]📋 PAPERS BY REGULATION[/bold cyan]",
            box=box.ROUNDED,
            show_header=True,
        )
    ...
    if stats.organized > 0:
        console.print()
        console.print("[bold cyan]📂 OUTPUT FOLDER STRUCTURE:[/bold cyan]")
        console.print(f"   [dim]{OUTPUT_DIR}/[/dim]")
        for reg_dir in sorted(OUTPUT_DIR.iterdir()):
            if reg_dir.is_dir():
                console.print(f"   ├── [yellow]{reg_dir.name}/[/yellow]")
                dept_dirs = sorted(reg_dir.iterdir())
                for i, dept_dir in enumerate(dept_dirs):
                    if dept_dir.is_dir():
                        files = list(dept_dir.glob("*.pdf"))
                        branch = "└──" if i == len(dept_dirs) - 1 else "├──"
                        console.print(f"   │   {branch} [cyan]{dept_dir.name}/[/cyan] ({len(files)} papers)")

    # Error log
    if stats.errors:
        console.print()
        console.print(f"[yellow]⚠️  {len(stats.errors)} errors encountered. First 10:[/yellow]")
        for err in stats.errors[:10]:
            console.print(f"   [red]• {err}[/red]")

    console.print()
    console.print("[bold green]🎉 All done! Papers organized in:[/bold green]", str(OUTPUT_DIR))
    console.print()
...
def main():
    ...
    try:
        ...
        console.print("\n[yellow]⚠️  Interrupted by user. Partial results saved.[/yellow]")
        sys.exit(1)
